...
# Usar pasta local do projeto: Extrações\KE5Z
pasta = os.path.join("Extrações", "KE5Z")
...
if not os.path.exists(pasta):
    ...
    print("Coloque os arquivos .txt na pasta Extrações/KE5Z/ do projeto")
    exit(1)
...
# Usar pasta local do projeto para KSBB: Extrações\KSBB
pasta_ksbb = os.path.join("Extrações", "KSBB")
...
if not os.path.exists(pasta_ksbb):
    ...
    print("Coloque os arquivos .txt na pasta Extrações/KSBB/ do projeto")
...
df_total[df_total['USI'].isin(['Veículos', 'TC Ext', 'LC'])].to_excel(caminho_veiculos, index=False)
...
df_total[df_total['USI'].isin(['PWT'])].to_excel(caminho_pwt, index=False)
